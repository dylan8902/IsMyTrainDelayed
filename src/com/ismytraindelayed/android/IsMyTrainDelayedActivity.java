...
public class IsMyTrainDelayedActivity extends Activity {
	...
	public void restoreSavedJourneys() {
		...
		if((settings.contains("saved_journeys"))&&(settings.getString("saved_journeys", null).length()>0)) {
	        ...
	        results.addView(journey_title, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
	        ...
	        for(int a = 0; a < journeys.length; a++) {
		        ...
		        results.addView(journey, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
	    ...
	    switch (item.getItemId()) {   
		    ...
		    case R.id.save_journey:
				...
				if(settings.contains("saved_journeys"))
					...
					saved_journeys = "";
				editor.putString("saved_journeys",saved_journeys.replace(from.getText()+","+to.getText()+";",""));
				editor.putString("saved_journeys",saved_journeys+from.getText()+","+to.getText()+";");
				editor.commit();
				Toast.makeText(getBaseContext(),"Journey Saved",5).show();
		    	return true;
		    
		    case R.id.delete_journey:
				SharedPreferences.Editor editor2 = settings.edit();	
		    	String saved_journeys2 = settings.getString("saved_journeys", null);
				editor2.putString("saved_journeys",saved_journeys2.replace(from.getText()+","+to.getText()+";",""));
		    	editor2.commit();
		    	Toast.makeText(getBaseContext(),"Removed Saved Journey",5).show();
		    	return true;
	...
	public boolean find_nearest(int field) {
		if(geolocation==null) {
			Toast.makeText(getBaseContext(),"Still acquiring your location, please wait",10).show();
			return true;
		}
		try {
			String query = "http://ismytraindelayed.com/stations.php?lat="+geolocation.getLatitude()+"&lng="+geolocation.getLongitude();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		button.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				try {
					String to_query = URLEncoder.encode(to.getText().toString());
					String from_query = URLEncoder.encode(from.getText().toString());
					String query = "http://ojp.nationalrail.co.uk/service/ldb/liveTrainsJson?departing="+departing+"&liveTrainsFrom="+from_query+"&liveTrainsTo="+to_query+"&serviceId=abcdefghijk&from="+from_query+"&to="+to_query;
					JSONObject response = new JSONObject(getJson(query));
					if(response.getString("error")=="Failed") {
				        ...
				        error_msg.setText("Sorry, unable to retrieve train times. Check network connection");
				        error.addView(error_msg);
			            results.addView(error, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
					}
					else {
					...
					if(trains.length()==0) {
				        ...
				        error_msg.setText("Sorry, no train information for that journey");
				        error.addView(error_msg);
			            results.addView(error, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
					}
					for (int i = 0; i < trains.length(); i++) {
			            ...
			            results.addView(row, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
			            //STATUS
			            ...
			            String status_string = Html.fromHtml(train.getString(3)).toString().replace("<br/>","-").replace("*","");
			            String platform = "";
			            if(train.getString(4).length()!=0) {
			            	platform = " (Platform "+train.getString(4).toString()+")";
			            }
			            status_text.setText(status_string+platform);
			            if((!(status_string.contains("On time")))&&(!(status_string.contains("Starts here"))))
			            	status_text.setTextColor(Color.RED);
			            status_text.setPadding(5,0,5,5);
			            status.addView(status_text);
			            results.addView(status, new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
					}
